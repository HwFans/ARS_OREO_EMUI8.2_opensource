...
unsigned long buf_avail_msec = 0;
unsigned long buf_null_msec = 0;
unsigned long q6asm_read_msec = 0;
unsigned long msec_long = 0;

#define MIN_PRINT_MSEC 20
				...
				uint32_t *index)
{
	...
	if (ac->io_mode & SYNC_IO_MODE) {
		...
		idx = port->cpu_buf;
		if (port->buf == NULL) {
			m_seconds =  jiffies_to_msecs(jiffies);
			msec_long = m_seconds - buf_null_msec;
			if(( msec_long > MIN_PRINT_MSEC) ||(buf_null_msec == 0)) {
				pr_err("%s: Buffer pointer null\n", __func__);
				buf_null_msec = m_seconds;
			}
		...
		if (port->buf[idx].used == dir) {
			...
			m_seconds =  jiffies_to_msecs(jiffies);
			msec_long = m_seconds - buf_avail_msec;
			if((msec_long > MIN_PRINT_MSEC) || (buf_avail_msec == 0)) {
				pr_err("%s: Next buf idx[0x%x] not available, dir[%d]\n",
						__func__, idx, dir);
				buf_avail_msec = m_seconds;
			}
			mutex_unlock(&port->lock);
			return NULL;
		}
			...
			int len)
{
	...
	if (ac->io_mode & SYNC_IO_MODE) {
		...
		if (port->buf == NULL) {
			m_seconds = jiffies_to_msecs(jiffies);
			msec_long = m_seconds - q6asm_read_msec;
			if((msec_long > MIN_PRINT_MSEC) || (q6asm_read_msec== 0)) {
				pr_err("%s: buf is NULL\n", __func__);
				q6asm_read_msec = m_seconds;
			}
			mutex_unlock(&port->lock);
			return -EINVAL;
		}
