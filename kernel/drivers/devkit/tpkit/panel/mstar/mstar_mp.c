...
void mp_dump_data(void *pBuf, u16 nLen, int nDataType, int nCarry, int nChangeLine, const char *desp)
{
    int i;
    char szStrTmp[10] = {0};
    char szStrBuf[512] = {0};
...
int msg30xx_short_judge(int *deltac_data)
{
    ...
    thrs = msg30xx_covert_Rvalue(dump_time, mp_test_data->sensorInfo.thrsICpin);
...
s8 msg30xx_open_judge_ratio(int deltac_size, int *by_pass_buf, int *result_buf)
{
    ...
    for (j = 0; j < deltac_size; j++)
    {
        ...
        if (result_buf[j] != NULL_DATA)
        {
			...
			ret = 1;
			mp_test_result->nNormalTestResultCheck[j] = (u16)(((j / drv_num) + 1) * 100 + ((j % drv_num) + 1));
			TS_LOG_ERR("open ratio fail [%d][%d]: %d, ratio_Max:%d, , ratio_Min:%d\n", j/sense_num, j%sense_num, mp_test_result->ratio_move[j], ratioAvg_border_max, ratioAvg_border_min);
                }
            }
            else if ((mp_test_data->Pattern_type == 3) && (mp_test_data->sensorInfo.numKey != 0) && ((j % drv_num == 0) || ((j + 2) % drv_num == 0)))
            {
    ...
    for (k = 0; k < deltac_size; k++)
    {
        ...
        if (result_buf[k] != NULL_DATA)
        {
            ...
            if (0 == check_thrs(result_buf[k], mp_test_data->Goldensample_CH_0_Max[k], mp_test_data->Goldensample_CH_0_Min[k]) && by_pass_buf[k] != NO_JUDGE)
            {
                nNormalTestResult[0] = 1;
                mp_test_result->nNormalTestResultCheck[k] = (u16)(((k / drv_num) + 1) * 100 + ((k % drv_num) + 1));
		TS_LOG_ERR("open fail channel[%d][%d]: %d, Golden_Max:%d, , Golden_Min:%d\n", k/sense_num, k%sense_num, result_buf[k], mp_test_data->Goldensample_CH_0_Max[k], mp_test_data->Goldensample_CH_0_Min[k]);
            }
            else
            {
                    mp_test_result->nNormalTestResultCheck[k] = PIN_NO_ERROR;
...
void msg30xx_uniformity_check_border_ratio(int *border_ratio)
{
    ...
    for(i = 0, index = 0; i <6 ; i++)
    {
         for(j = 0; j <sense_num ; j++, index ++)
        {
            TS_LOG_DEBUG("%d,",border_ratio[i*sense_num+j]);
        }
    }
}
   ...
   for(i = 0, index = 0; i < sense_num ; i++)
    {
        ...
        {
            if(tskit_mstar_data->mstar_chip_data->ic_type == CHIP_TYPE_MSG2846A){
                if(c_buf[index] == NOTCH_JUDGE){//set judge data
        ...
        if(i==0){//top
            ...
            if(delta_LR_buf[index] < 0 && delta_LR_buf[index] != PIN_NO_ERROR) {
                TS_LOG_ERR("%s: Failed : delta_LR_buf[%d] = %d  \n", __func__, index, delta_LR_buf[index]);
    ...
    TS_LOG_DEBUG("%s:  delta_LR_buf\n",__func__);
    for(i = 0, index = 0; i <sense_num ; i++)
    {
        for(j = 0; j <drv_num ; j++, index ++)
        {
            TS_LOG_DEBUG(KERN_CONT"%d,\n",delta_LR_buf[index]);
        }
    }

    TS_LOG_DEBUG("%s: delta_UD_buf\n",__func__);
    for(i = 0, index = 0; i <sense_num ; i++)
    {
        for(j = 0; j <drv_num ; j++, index ++)
        {
...
int msg30xx_uniformity_judge(int *deltac_data, int deltac_size)
{
    ...
    TS_LOG_DEBUG("%s: mstar deltac_data_buf \n",__func__);
    for(i = 0, index = 0; i <sense_num ; i++)
    {
        for(j = 0; j <drv_num ; j++, index ++)
        {
    ...
    TS_LOG_DEBUG("%s: border_ratio_buf\n",__func__);
    for(i = 0, index = 0; i <6 ; i++)
    {
        for(j = 0; j <sense_num ; j++, index ++)
        {
            TS_LOG_DEBUG(KERN_CONT"%d,\n",border_ratio_buf[i*sense_num+j]);
        }
    }
    if(tskit_mstar_data->mstar_chip_data->ic_type == CHIP_TYPE_MSG2846A){
...
static int new_get_raw_data(s16 *pRawData, u16 count, u8 dump_time)
{
    ...
    mp_dump_data(pShotOriData, data_length * 2 + touch_info_length, 8, 16, 16, "original raw");
...
static int new_get_deltac(int *delt, u8 dump_time)
{
    ...
    mp_dump_data(delt, count, -32, 10, 16, "DeltaC");
...
static int new_send_test_cmd(u16 fmode, int mode)
{
    ...
    mp_dump_data(cmd, ARRAY_SIZE(cmd), 8, 16, 16, "Test command");
...
static int new_flow_start_test(void)
{
    ...
    if (mp_test_data->run_type == RUN_SHORT_TEST) {
        ...
        mp_dump_data(deltac_data, MAX_SUBFRAMES_30XX * MAX_AFENUMs_30XX, -32, 10, 10, "short dump2 - dump1");
    }
...
static void mp_calc_golden_range(int *goldenTbl, u16 weight, u16 golden_up, int *maxTbl, int *minTbl, int length)
{
    ...
    mp_dump_data(goldenTbl, allnode_number, -32, 10, 16, "goldenTbl");
    ...
    mp_dump_data(maxTbl, allnode_number, -32, 10, 16, "maxTbl");
    mp_dump_data(minTbl, allnode_number, -32, 10, 16, "minTbl");
}
...
int mp_parse(char *path)
{
    ...
    if(IS_ERR_OR_NULL(f)) {
        TS_LOG_ERR("Failed to open the file at %ld. retry\n", PTR_ERR(f));
        return -ENOENT;
    }
...
if(tskit_mstar_data->mstar_chip_data->ic_type == CHIP_TYPE_MSG2846A){
    ...
    mp_test_data->SHORT_sensor_pin_number= ms_atoi(str);
    ms_get_ini_data("SHORT_TEST_N", "SHORT_SAMPLE_NUM", str);
    mp_test_data->SHORT_sample_number = ms_atoi(str);
}
...
static int mp_main_init_var(void)
{
    ...
    {
        TS_LOG_ERR("Failed to allocate UniformityTest mem\n");
        return -1;
    }
...
void mp_send_data2kit(struct ts_rawdata_info *info)
{
    ...
    if ((!ERR_ALLOC_MEM(mp_test_result)) && (!ERR_ALLOC_MEM(mp_test_result->pOpenResultData)) &&
        ...
        for (i = 0; i < drv_num * sense_num; i++) {
            if (i < 2) {
                TS_LOG_INFO("mp_test_result->pOpenResultData[%d] = %d\n", i, mp_test_result->pOpenResultData[i]);
            }
            info->buff[buff_index++] = mp_test_result->pOpenResultData[i];
        }
        for (i = 0; i < drv_num * sense_num; i++) {
        if (i < 2) {
                TS_LOG_INFO("mp_test_result->pGolden_CH[%d] = %d\n", i, mp_test_result->pGolden_CH[i]);
            }
        ...
        if(tskit_mstar_data->mstar_chip_data->ic_type == CHIP_TYPE_MSG28XXA){
            for (i = 0; i < drv_num * sense_num; i++) {
                if (i < 2) {
                    TS_LOG_INFO("mp_test_result->pGolden_CH_Max_Avg[%d] = %d\n", i, mp_test_result->pGolden_CH_Max_Avg[i]);
                }
        ...
        for (i = 0; i < drv_num + sense_num; i++) {
            if (i < 2) {
                TS_LOG_INFO("mp_test_result->pShortResultData[%d] = %d\n", i, mp_test_result->pShortResultData[i]);
            }
        ...
        for (i = 0; i < drv_num * sense_num; i++) {
            if (i < 2) {
                TS_LOG_INFO("mp_test_result->puniformity_deltac[%d] = %d\n", i, mp_test_result->puniformity_deltac[i]);
            }
        ...
        for (i = 0; i < drv_num * sense_num; i++) {
            if (i < 2) {
                TS_LOG_INFO("mp_test_result->pdelta_LR_buf[%d] = %d\n", i, mp_test_result->pdelta_LR_buf[i]);
            }
        ...
        for (i = 0; i < drv_num * sense_num; i++) {
            if (i < 2) {
                TS_LOG_INFO("mp_test_result->pdelta_UD_buf[%d] = %d\n", i, mp_test_result->pdelta_UD_buf[i]);
            }
        ...
        for (i = 0; i < 6 * sense_num; i++) {
            if (i < 2) {
                TS_LOG_INFO("mp_test_result->pborder_ratio_buf[%d] = %d\n", i, mp_test_result->pborder_ratio_buf[i]);
            }
            info->buff[buff_index++] = mp_test_result->pborder_ratio_buf[i];
        }
    }else{
           TS_LOG_ERR("%s, Failed to allocate mem force change data to 0\n", __func__);
        for (i = 0; i < drv_num * sense_num; i++) {
